import { TbRewindBackward15 } from "react-icons/tb"

function Footer() {
    return (
        <>
            <div className='flex flex-row justify-between items-center pl-6 pr-6 md:pl-12 md:pr-12 min-h-[150px] bg-[#5C8374] text-[#FFF5E4]'>
                <div className='flex flex-row items-center'>
                    <span className="font-mono text-4xl font-extrabold">DIRECTION</span>
                    <TbRewindBackward15 size={40} />
                </div>
                <div className='flex flex-row gap-12 text-xl items-center hidden md:flex'>
                    <span>Goal</span>
                    <span>Expanse</span>
                    <span>Note</span>
                </div>
                <div className='p-3 flex justify-center items-center flex-col text-xl'>
                    Sign IN
                </div>
            </div>
        </>
    )
}

export default Footer